...
data OS = OsLinux | OsOSX | OsWindows               deriving (Show,Read,Eq,Ord)
data Arch = ArchI386 | ArchX86_64                   deriving (Show,Read,Eq,Ord)
data DistType = DistBinary OS Arch | DistSource     deriving (Show,Read,Eq,Ord)
data Variant = Standard | Full | Minimal            deriving (Show,Read,Eq,Ord)

type Url = String
type Hash = String

data Month = Month { _year, _month :: Int }
...
monthName m = mnames !! (_month m -1)

monthYear :: Month -> String
monthYear m = monthName m ++ " " ++ show (_year m)
...
isSource :: FileInfo -> Bool
isSource file = 
  case _disttype file of
    DistSource -> True
    _          -> False

isBinaryFor :: OS -> FileInfo -> Bool
isBinaryFor wantedOS file =
  case _disttype file of
    DistSource       -> False
    DistBinary os _  -> os == wantedOS

varpart Standard = ""
varpart Minimal  = "Minimal"
varpart Full     = "Full"

osbits :: DistType -> String
osbits (DistSource) = ""
osbits (DistBinary os ar) = show (archBits ar)

osname :: DistType -> String
osname DistSource = ""
osname (DistBinary os ar) = osName os
...
hashCode :: FileInfo -> String
hashCode file = maybe "" id (_hash file)
...
-- compare release months (in reverse order)
comparingMonth :: Release -> Release-> Ordering
comparingMonth = flip (on compare _rls_month)

sameYear :: Release -> Release -> Bool
sameYear = on (==) (_year . _rls_month)

-- sort and group releases for display on the Prior Releases page
groupBySameYear :: [Release] -> [[Release]]
groupBySameYear releases =   groupBy sameYear $ sortBy comparingMonth $ releases

-- constructors

jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, dec :: Int -> Month
[ jan, feb, mar, apr, may, jun, jul, aug, sep, oct, nov, dec ]
  = map (\m y -> Month y m) [1..12]

lin, mac, win :: Arch -> Url -> Maybe Hash -> FileInfo
lin a u mh = FileInfo (DistBinary OsLinux a)   u mh Standard
mac a u mh = FileInfo (DistBinary OsOSX a)     u mh Standard
win a u mh = FileInfo (DistBinary OsWindows a) u mh Standard

full :: FileInfo -> FileInfo
full f = f { _variant = Full }

mini :: FileInfo -> FileInfo
mini f = f { _variant = Minimal }
...
src u mh = FileInfo DistSource u mh Standard
...
releaseFiles = 
  ...
  in map rl
      [
       ("8.0.1", may 2016,
        [ mini $ lin x86_64    "download/8.0.1/haskell-platform-8.0.1-unknown-posix--minimal-x86_64.tar.gz" $ sha256 "adec8e8f2e2440d7f506f1cb9aaf20496cd443660e55c0d588f28a0119171f8a"
        , full $ lin x86_64    "download/8.0.1/haskell-platform-8.0.1-unknown-posix--full-x86_64.tar.gz" $ sha256 "d747aaa51eb20a7c8b4de93fa2a0d07c3b54fc5f36bf50fcede1a332812656f7"
        , mini $ lin i386      "download/8.0.1/haskell-platform-8.0.1-unknown-posix--minimal-i386.tar.gz" $ sha256 "1476ec7fda53654fe97118ded44333b091160fc5f4588c2ad7a0f8145c254d14"
        , full $ lin i386      "download/8.0.1/haskell-platform-8.0.1-unknown-posix--full-i386.tar.gz" $ sha256 "4643123f51401489d99302c150dc763f1d92614c428b921257b375f3895f7a79"
        , mini $ mac x86_64    "download/8.0.1/Haskell%20Platform%208.0.1%20Minimal%2064bit-signed.pkg" $ sha256 "2abc58b2bbdd8df17769e69ebec3507987f31c2cb3e49f0224c74af85a3d4960"
        , full $ mac x86_64    "download/8.0.1/Haskell%20Platform%208.0.1%20Full%2064bit-signed.pkg " $ sha256 "3ad7ea7afa5ddb9d58db7e05d1e650f30cb7982067dd1fac75b62c5b071fda4d"
        , mini $ win i386      "download/8.0.1/HaskellPlatform-8.0.1-minimal-i386-setup.exe" $ sha256 "6755ede367b70a2f714cd014a0eed096f9ccff6fd036c21d4d1e9b7f8f922a07"
        , full $ win i386      "download/8.0.1/HaskellPlatform-8.0.1-full-i386-setup.exe" $ sha256 "88a01bfa5b94f4c6f4d9959fc1b711acb944799b6090a1d8004a0f42f320ee9b"
        , mini $ win x86_64    "download/8.0.1/HaskellPlatform-8.0.1-minimal-x86_64-setup.exe" $ sha256 "8115610a1042c744e1e652b0db97f8d127b5527aef1dc1b1674d80b0d8575c7e"
        , full $ win x86_64    "download/8.0.1/HaskellPlatform-8.0.1-full-x86_64-setup.exe" $ sha256 "0a22f668e6c03077c8f3605846f3e0c5eea4e12e0cc0595ddf7bc0494cf65200"
        , full $ src           "download/8.0.1/haskell-platform-8.0.1.tar.gz" $ sha256 "38af99a9ae4afce56df75a753a19e7a4986bfbc8ce22f93b8308b5ab9e5a19c6"
        ])
     , ("7.10.3", dec 2015,
